...
export default function Header({ personalInfo }) {
  ...
  const { name, title, socialLinks } = personalInfo;
  ...
  const getSocialSvg = (platform) => {
    const svgs = {
      ...
      linkedin: (
        <svg className="h-6 w-6 text-[#4b6b8b]" fill="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <header className="animate-slide-in-up" style={{ '--delay': '0s' }}>
      <div className="hud-panel" id="info">
        <img src="assets/pic.jpg" id="picture" alt={name} />
        <div>
          <h1 id="name">{name.toUpperCase()}</h1>
          <h2 id="title">{title.toUpperCase()}</h2>
          <div className="hud-panel-social" id="social-links">
            {Object.entries(socialLinks).map(([platform, linkData]) => (
              <a
                key={platform}
                href={linkData.url}
                target="_blank"
      ...
      <div className="launch-blog-container">
        <a
          className="launch-button"
          href="assets/resume.pdf"
          target="_blank"
          rel="noopener noreferrer"
          download="Saqib_Ali_Shah_Resume.pdf"
        >
          RESUME
        </a>
      </div>
